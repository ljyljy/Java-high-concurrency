package com.ljy.mutithread.basic.create3;
...
public class CreateThreadPoolDemo {
    ...
    public void testThreadPoolExecutor() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            executor.execute(() -> {
                ...
                try { // 反例：↓ 5个线程，第一个线程永远无法完成
                    // 由于后面4个线程会优先进入阻塞队列中等待（4<100=workQueueSize）
                    // 因此线程池中，永远只有线程1执行，线程2-5无法执行
    ...
    public void testCustomIgnorePolicy() {
        ThreadPoolExecutor pool = new ThreadPoolExecutor(
                2, //核心线程数
                4, //最大线程数, TODO: test 4, >4(可能不拒绝，如>=6)
                100, TimeUnit.MILLISECONDS, // keepAliveTime：如果当前线程池中线程数大于corePoolSize。
                // * 多余的线程，在等待keepAliveTime时间后如果还没有新的线程任务指派给它，它就会被回收
                new ArrayBlockingQueue<>(2),//最大排队任务数
                new SimpleThreadFactory(), //线程工厂
                new CustomIgnorePolicy()//拒绝和异常策略
        ) {
            @Override
    ...
    public void testHooks() {
        ...
        for (int i = 0; i < 5; i++) {
//            Future<?> submit = pool.submit(new TargetTask());
            pool.execute(new TargetTask());
        }
    ...
    public void testSubmit() {
        ...
        pool.execute(new TargetTaskWithError()); // 注释掉，以与submit进行对比！
        /**
        ...
        try {
            ...
            if (future.get() == null) {
                ...
                Print.tco("submit - 任务完成");
            }
        } catch (Exception e) {
            Print.tco("submit - " + e.getCause().getMessage());
        }
    ...
    //测试用例：优雅关闭
    @Test
    public void testShutdownGracefully2() {
        ScheduledExecutorService pool = Executors.newScheduledThreadPool(2);
        pool.shutdown();
        try {
            for (int i = 0; i < 1000; i++) { //循环关闭 1000 次，每次等待 100 毫秒
                // 若线程池完成关闭，则返回true
                if (pool.awaitTermination(100, TimeUnit.MILLISECONDS)) {
                    break;
                }
                pool.shutdownNow();// 调用 shutdownNow 取消正在执行的任务
            }
        } catch (InterruptedException e) {
            e.printStackTrace();
