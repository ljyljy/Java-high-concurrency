...
public class InnerLockTest {
    ...
    // objectLock1占用16B = 对象头(4B*3) + 对象体(一个int=4B*1) + padding(0)=16+0
    @org.junit.Test
    ...
    @org.junit.Test
    public void showNoLockObject2() throws InterruptedException {
        //输出JVM的信息
        Print.fo(VM.current().details());
        //创建一个对象
...
//                        Print.consoleInput();
        ...
        Runnable runnable = () -> {
            for (int i = 0; i < MAX_TURN; i++) {
                synchronized (lock) {
                    lock.increase();
                    if (i == 1) {
                        Print.tcfo("第一个线程占有锁, lock 的状态: 【偏向锁】");
                        lock.printObjectStruct();
                    }
                }
        ...
        Runnable lightweightRunnable = () -> {
            for (int i = 0; i < MAX_TURN; i++) {
                synchronized (lock) {
                    lock.increase();
                    if (i == MAX_TURN / 2) {
                        Print.tcfo("第二个线程占有锁, lock 的状态: 【轻量级锁】");
                        lock.printObjectStruct();
                    }
                    //每一次循环等待1ms
                    sleepMilliSeconds(1);
    ...
    public void showHeavyweightLock() throws InterruptedException {
        ...
        Print.tcfo("抢占锁前, counter 的状态: 【偏向锁】");
        ...
        Runnable runnable = () -> {
            for (int i = 0; i < MAX_TURN; i++) {
                synchronized (counter) {
                    counter.increase();
                    if (i == 0) {
                        Print.tcfo("第一个线程占有锁, counter 的状态: 【偏向锁】");
                        counter.printObjectStruct();
                    }
                }
        ...
        Runnable lightweightRunnable = () -> {
            for (int i = 0; i < MAX_TURN; i++) {
                synchronized (counter) {
                    counter.increase();
                    if (i == 0) {
                        if (getCurThreadName().equals("抢锁线程1")) {
                            Print.tcfo("占有锁, counter 的状态: 【抢锁线程1st:轻量级锁】");
                        } else { // （与for次数MAX_TURN有关，若for 10次，则都是轻量级锁！）
                            Print.tcfo("占有锁, counter 的状态: 【抢锁线程2nd:重量级锁】");
                        }
                        counter.printObjectStruct();
                    }
                    //每一次循环等待10ms
        ...
        latch.await();
        sleepMilliSeconds(2000);  //等待2s
        Print.tcfo("释放锁后, counter 的状态: 【无锁】");
        counter.printObjectStruct();
    }
