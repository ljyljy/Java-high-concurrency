...
public class TwoLockDemo {
    ...
    public static void tryTowLock(Lock lock1, Lock lock2) {
        ...
        if (lock1Succeed) {
            Print.synTco(" ^-^抢到了外部锁1, 锁为：" + lock1Name);
        } else {
            Print.synTco(" @-@超时中断，抢占外部锁1失败, 锁为：" + lock1Name);
            return;
        }
        try {
            Print.synTco(" -- 开始抢占内部锁2, 锁为：" + lock2Name);
            try {
                //等待一个10s秒的随机数
            ...
            if (lock2Succeed) {
                Print.synTco(" ^-^抢到了内部锁2, 锁为：" + lock2Name);
            } else {
                Print.synTco(" @-@超时中断，抢占内部锁2失败, 锁为：" + lock2Name);
                return;
            }
            try {
                Print.synTco(" do something ");
                ...
                lock2.unlock();
                Print.synTco(" 释放了内部锁2, 锁为：" + lock2Name);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            lock1.unlock();
            Print.synTco(" 释放了外部锁1, 锁为：" + lock1Name);
        }
    }
