...
import static com.ljy.util.DateUtil.getNowTime;
...
public class StampedLockTest {
    ...
    public static void main(String[] args) throws InterruptedException {
        //创建Runnable可执行实例
        Runnable writeTarget = () -> put("key", "value"); // 对应 写线程
        Runnable readTarget = () -> optimisticRead("key0");// 对应 读线程1
        put("key0", "value0"); // 主线程
        Runnable readTarget2 = () -> optimisticRead("key");

        //创建1条读线程
        new Thread(readTarget, "读线程1").start();
        //创建1条写线程，并启动
        new Thread(writeTarget, "写线程").start();
        //创建1条读线程
        new Thread(readTarget2, "读线程2").start();
    }
}